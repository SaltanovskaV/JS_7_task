...
function validateFileType(str){
 return str = RegExp(/.+(\.jpg|\.jpeg|\.png)$/).test(str);
}
...
function checkYear(str){
 return str = RegExp(/^(19\d\d|20\d\d|2100)$/).test(str);
}
...
function checkEmail(str) {
 return str = RegExp(/^[a-zA-Z-.]+[a-zA-Z]+@[a-zA-Z\.]+\.+[a-z]{2,3}$/).test(str);
}
...
return str = RegExp(/^(http:|https:)\/\/([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/).test(str);
}
